...
function updateControls(){
  ...
  $('edit-module').disabled=busy||!report;$('edit-mode').disabled=busy||rawEdited||!report;
...
function updateFiles(){
  ...
  $('result-state').textContent=rawEdited?'输入已修改 · 图中仍是上次布局':dirty?'未保存预览 · 请评估并保存':report?'正在查看已保存结果':'已保存搜索记录 · 没有可展示的布局';
  $('result-location').textContent=saved?currentFolder():'尚未保存到工程';
  $('result-note').textContent=dirty||rawEdited?'路径指向上次结果。点击“评估并保存”保存当前输入及新报告。':'implementation-difficulties.md 为实现难点清单，report.html 为完整报告，layout-plan.yml 为规划方案；output-manifest.json 说明文件及单位。';
  ...
  showMethods(report,searchResult);updateControls();updateAssessment();
}
function renderResult(result,mode){
  activeStorage=result.storage||null;storageMode=mode;
  if(mode==='optimize'){
    searchResult=result;baseline=result.baseline;$('candidate').innerHTML=(baseline?'<option value="base">给定布局 / 基线</option>':'')+result.candidates.map((r,i)=>`<option value="${i}">候选 ${i+1} · ${r.candidate.accepted?'通过当前模型约束':'需修改 / 数据不全'}</option>`).join('');
    if(result.candidates.length){activeCandidate=savedCandidate='0';$('candidate').value='0';show(result.candidates[0],true);}else if(baseline){activeCandidate=savedCandidate='base';$('candidate').value='base';show(baseline,true);}comparison(result);
  }else{searchResult=null;baseline=result;activeCandidate=savedCandidate='current';$('candidate').innerHTML='<option value="current">给定布局</option>';$('comparison').hidden=true;show(result,true);}updateFiles();
}
...
async function loadProject(id){
  ...
  try{const response=await fetch(`/api/projects/${id}/input`);if(!response.ok)throw new Error('项目输入读取失败');const input=await response.text();currentProjectId=id;localStorage.setItem('resim-project',id);
    ...
    if(latest){
      ...
      if(match!==null){renderResult(payload.result,payload.mode);activeCandidate=savedCandidate=match;$('candidate').value=match;show(working.report,true);if(searchResult)comparison(searchResult);$('history').value=latest.run_id;notice('已载入工程当前输入，对应已保存方案。');}
      else{dirty=true;activeCandidate='draft';$('candidate').innerHTML='<option value="draft">当前工程输入 · 未保存预览</option>';show(working.report,true);notice('已按当前工程输入重新预览。当前输入或计算版本与最近报告不同，点击“评估并保存”生成新记录；历史报告可单独查看。');}
    }else{dirty=true;activeCandidate='draft';$('candidate').innerHTML='<option value="draft">当前工程输入 · 未保存预览</option>';show(working.report,true);notice('已预览当前输入，点击“评估并保存”创建运行记录。');}
  ...
  }catch(e){openWorkspace('inputs');$('editor-panel').hidden=false;notice('当前输入尚不能展示：'+e.message+'。可在输入编辑器中修改；没有完整布局时可运行自动规划。',true);}finally{setBusy(false);}
}
...
function renderIssues(){
  ...
  $('issue-count').textContent=`${items.length} / ${report.issues.length} 项`;
  ...
  $('issues').querySelectorAll('[data-issue]').forEach(button=>button.onclick=()=>{
    const issue=report.issues[Number(button.dataset.issue)];selectedIssue=issue;
    ...
    else{const channel=report.routing_channels?.find(c=>c.id===issue.subject);const d=report.dies.find(d=>issue.subject===d.id||issue.subject.startsWith(d.id+':')||d.id===channel?.die);if(d)$('layer').value=d.id;if(issue.code==='METAL_CAPACITY')$('metal-layer').value=issue.subject.slice(issue.subject.indexOf(':')+1);}
    $('focused-issue').textContent=`${issue.subject}：${issue.reason}。建议：${issue.suggestion}`;$('focused-issue').hidden=false;
...
function show(value,fit=false){
  ...
  $('edit-module').innerHTML='<option value="">选择模块</option>'+report.modules.map(m=>`<option value="${esc(m.id)}">${esc(m.id)} · ${esc(m.die)}</option>`).join('');
...
function updateLive(){
  ...
  $('draft-status').textContent=rawEdited?'输入待应用':previewFailed?'预览失败，可重试或撤销':previewPending?'移动后重算中':dirty?'未保存预览 · 评估并保存为新版本':'已评估布局';updateControls();
...
function inspect(m,filter=true){
  ...
  $('detail').insertAdjacentHTML('beforeend','<button id="edit-selected" class="text-button">编辑此模块的位置 ↗</button>');
  $('edit-selected').onclick=()=>{openWorkspace('layout');$('layout-tools').open=true;$('layout-tools').scrollIntoView({behavior:'smooth',block:'start'});$('edit-x').focus({preventScroll:true});};
  updateControls();
}
function comparison(result){
  $('comparison').hidden=false;const c=compareReports(result.baseline,report);if(!c){$('comparison').innerHTML='<h3>搜索结果</h3><p>'+esc(result.message||result.search_status)+'</p>';return;}
  const names={errors:'约束违例',unknowns:'缺失数据',weighted_delay_ps:'加权延迟估算 / ps（非执行时间）',weighted_wirelength_um:'加权线长 / wire·µm',signal_via_segments:'信号 TSV 需求 / 逐接口',total_via_segments:'全部 TSV',peak_congestion:'峰值拥塞',power_W:'总功耗 W'};
  $('comparison').innerHTML='<h3>基线 → '+esc($('candidate').selectedOptions[0]?.textContent||'当前方案')+'</h3><table><tr><th>指标</th><th>基线</th><th>当前选中方案</th><th>变化</th></tr>'+Object.entries(names).map(([k,v])=>`<tr><td>${v}</td><td>${fmt(c[k].baseline,4)}</td><td>${fmt(c[k].candidate,4)}</td><td>${c[k].delta>0?'+':''}${fmt(c[k].delta,4)}</td></tr>`).join('')+`</table><p>迁移模块：${esc(c.migrated_modules.join('、')||'无')}。耗时 ${fmt(result.elapsed_s,1)} 秒；未声明全局最优。</p>`;
}
...
function setDraft(project,remember=true){
  ...
  if(!$('candidate').querySelector('[value="draft"]'))$('candidate').insertAdjacentHTML('beforeend','<option value="draft">手动编辑 · 未保存</option>');$('candidate').value=activeCandidate='draft';$('comparison').hidden=true;$('wires').checked=true;
...
async function preview(){
  ...
  try{const response=await api('layout/preview',clone(report.project),previewController.signal);if(token!==revision)return;previewPending=false;$('yaml').value=response.yaml;show(response.report);notice(`布局预览已更新：${response.report.summary.errors} 项违例，${response.report.summary.unknowns} 项缺失。评估并保存后生成新结果。`);}
...
async function applyMove(){
  ...
  try{const x=$('edit-x').value,y=$('edit-y').value;if(x===''||y==='')throw new Error('请填写 X 和 Y 坐标');const target=$('edit-die').value;setDraft(movePlacement(report.project,selectedId,target,Number(x),Number(y)));if($('layer').value!=='all')$('layer').value=target;viewer?.draw(report,true);await preview();}catch(e){notice(e.message,true);}
}
let viewer;
try{viewer=new Viewer({selectPort:showPort,selectTSV:showTSV,select:id=>{if(viewer)viewer.selectedLink=null;if(connectionPane)connectionPane.selected=null;inspect(report.modules.find(m=>m.id===id));viewer.draw(report,previewPending||previewFailed);},selectLink:id=>connectionPane?.select(id),editable:()=>Boolean(report&&!busy&&!rawEdited&&$('edit-mode').checked),start:rememberMove,move:(id,die,x,y)=>{try{setDraft(movePlacement(report.project,id,die,x,y),false);clearTimeout(previewTimer);previewTimer=setTimeout(preview,180);}catch(e){notice(e.message,true);}},finish:preview,error:message=>notice(message,true)});}catch(e){$('render-error').hidden=false;$('render-error').textContent='3D 渲染不可用：'+e.message;}
connectionPane=new Connections(id=>{
  if(id){
    $('wires').checked=true;const link=report.project.architecture.links.find(l=>l.id===id),count=linkBudget(link);
...
function setView(mode){if(mode==='2d'&&report&&$('layer').value==='all')$('layer').value=report.modules.find(m=>m.id===selectedId)?.die||report.dies[0].id;if(mode==='3d'){$('layer').value='all';$('edit-mode').checked=false;}viewer?.setView(mode,report,previewPending||previewFailed);}
...
$('edit-module').onchange=()=>{const m=report?.modules.find(m=>m.id===$('edit-module').value);inspect(m);if(m){if($('layer').value!=='all')$('layer').value=m.die;viewer?.draw(report,previewPending||previewFailed);viewer?.reset();}};
...
function showTSV(id){
  ...
  viewer.selectedPort=null;viewer.selectedTSV=id;
  $('port-popover').hidden=false;
  $('port-popover-content').innerHTML='<h3>跨层接口</h3><b>'+esc(v.id)+'</b>'+[
    ['连接层',v.lower_die+' ↔ '+v.upper_die],['接口 / 键合',(v.interconnect||'TSV')+' / '+(v.orientation||'未定义')],['通路预算说明',(v.budget_paths||[]).join('；')||'见关联连接'],['所属 core',v.region?.core||'共享'],
    ['区域尺寸',fmt(v.region?.width_um)+' × '+fmt(v.region?.height_um)+' µm'],
  ...
  ].map(([k,x])=>`<div class="detail-row"><span>${esc(k)}</span><b>${esc(x)}</b></div>`).join('')+'<p>柱体为 TSV 区域示意，显示柱数不代表实际数量；未知预算不会计为零。</p>';
...
function refreshSceneUI(){
  ...
  $('external-control').hidden=!single;
  ...
  $('scene-dimensions').textContent=report.dies.map(d=>`${d.id}: ${fmt(d.width_um/1000)} × ${fmt(d.height_um/1000)} mm · ${fmt(d.area_mm2)} mm²`).join('　 |　 ')+'。DRAM 仅展示 die 外形与 TSV 接口，内部模块隐藏；资源预算仍使用工程输入。';
  if(report.project.architecture.chip==='blx_scheme1')$('scene-context').textContent='16 core 重复堆叠 · DRAM—TSV—B（正面↓）—HB/F2F—L（正面↑）—TSV/B2B—X（正面↓） · 整体显示核轮廓，点击核查看细节 · 模块为占位';
  const peers=peerConnections(report.project,core);$('external-peers').hidden=!single;
  $('peer-list').innerHTML=peers.length?peers.map(p=>`<article><b>${esc(p.local.replace(core+'__','').toUpperCase())}</b><span>↔ ${esc(p.core)} / ${esc(p.remote.replace(p.core+'__',''))}</span><div>${[...p.outgoing.map(l=>[l,'发送 →']),...p.incoming.map(l=>[l,'← 接收'])].map(([l,d])=>`<button data-peer-link="${esc(l.id)}">${d} ${fmt(l.bus_width_bits,0)} bit</button>`).join('')}</div></article>`).join(''):'<p>当前输入没有该 core 的外部连接。</p>';
  $('peer-list').querySelectorAll('[data-peer-link]').forEach(b=>b.onclick=()=>{const l=report.project.architecture.links.find(l=>l.id===b.dataset.peerLink),local=report.modules.find(m=>m.id===l.source&&m.core===core)||report.modules.find(m=>m.id===l.target&&m.core===core);$('show-external').checked=true;$('layer').value=local.die;inspect(null);connectionPane.select(l.id);viewer?.reset();$('layout-workbench').scrollIntoView({behavior:'smooth',block:'start'});});
}
...
$('nav-rotate').onclick=()=>{$('edit-mode').checked=false;if(viewer?.view==='2d')viewer.setView('3d',report,previewPending||previewFailed);viewer?.setNavigation('rotate');};
$('scene-scope').onchange=changeScene;$('scene-core').onchange=changeScene;
$('clear-selection').onclick=()=>{inspect(null);viewer?.draw(report,previewPending||previewFailed);};
...
function renderDifficulties(){
  ...
  for(const [index,i] of report.issues.entries()){
