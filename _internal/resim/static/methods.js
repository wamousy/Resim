...
export function showMethods(report,searchResult=null){
  ...
  $('model-methods').innerHTML=`<div class="panel-head"><div><div class="section-label">工艺与参数依据</div><h2>${esc(resources.technology)}</h2></div><span>${resources.metals.length} 个金属层 · 当前工程输入</span></div>
    <p class="method-source">来源声明：${esc(resources.technology_provenance)}</p><div class="method-grid">
    <article><h3>实际使用哪些工艺数据</h3><p>${esc(m?.technology.scope||'历史报告：使用已保存输入中的面积、金属线宽与 pitch 参数。')}</p><p>${esc(m?.technology.builtin_reference||'内置参考记录可从下方入口查看。')}</p>${resources.technology.toLowerCase().includes('nangate45')?'<p>内置多层示例的 SRAM/DRAM 尺寸、功耗和 TSV 参数是演示假设，并非 Nangate45 已标定工艺数据。</p>':''}<details class="source-notes"><summary>工程原始来源与备注</summary><p>TSV 来源：${esc(resources.tsv?.provenance||'未提供')}</p><p>${esc((resources.notes||[]).join('；'))}</p></details><a href="/api/sources" target="_blank" rel="noreferrer">查看内置参考文件的版本与 SHA-256 ↗</a><small>内置参考记录用于追溯示例；当前工程参数以输入为准。</small></article>
    <article><h3>目标利用率是谁设定的？</h3><b class="method-value">${esc(targets)} <small>本工程目标值</small></b><p>${esc(m?.utilization.ownership||'target_cell_utilization 为项目规划输入，省略时默认 65%；不是工艺库规定。')}</p><p>${esc(m?.utilization.definition||'实际利用率 = 标准单元面积 / (模块矩形面积 − 硬宏面积)。')}</p><p>建议在模块的 utilization_basis 中记录设定人或校准依据；模块详情可查看目标值和实际值。</p></article>
    <article><h3>信号 TSV 需求（逐接口累加）</h3><code>Σ（物理信号线数 × 跨越接口数）</code><p>${esc(m?.signals.example||'64 根线跨越两个相邻接口，累计需求为 128 个信号 TSV 接口位置。')}</p><p>${esc(m?.signals.exclusions||'包含数据、控制与冗余，不含电源和地；不是独立逻辑连接数，也不是已放置孔数。')}</p></article>
    <article><h3>连接端点与路径口径</h3><p>${esc(m?.routing.description||'本历史报告未记录边缘端口模型，线长与路径保留当时结果；重新评估才使用当前模型。')}</p><p>端口位置随模块移动和跨层迁移更新，路径变化后同步重算互联面积与拥塞。</p></article></div>`;
  ...
  $('search-method').innerHTML=`<div class="panel-head"><div><div class="section-label">寻优依据</div><h2>自动分区与布局 · ${esc(algorithm.algorithm)}</h2></div><span>网格 ${fmt(algorithm.grid_um)} µm · 时间上限 ${fmt(algorithm.time_limit_s)} s · 最多 ${algorithm.candidates} 个候选</span></div>
    ...
    <details class="algorithm-details" open><summary>目标、约束与“最优”的含义</summary><code>${esc(algorithm.objective)}</code><p>当前权重：线长 ${algorithm.wirelength_weight}；跨层 ${algorithm.tier_crossing_weight}。相对差距 = (目标值 − 下界) / max(|目标值|, 1)。</p><p><b>求解约束：</b>${esc(algorithm.constraints)}</p><p><b>复核与排名：</b>${esc(algorithm.ranking)}</p><p><b>适用范围：</b>${esc(algorithm.limitations)}</p><p class="optimality-note">${esc(algorithm.optimality)}</p><p>${esc(algorithm.improvement)}</p></details>
